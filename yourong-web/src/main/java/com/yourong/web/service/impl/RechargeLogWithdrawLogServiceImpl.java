...
@Service
public class RechargeLogWithdrawLogServiceImpl extends BaseService implements RechargeLogWithdrawLogService {
    ...
    public int updateStateByOutTradeNo(BigDecimal amount, int status,String rechargeNo, String outTradeNo) {
        try {
            return rechargeLogManager.updateStateByOutTradeNo(amount, status,rechargeNo, outTradeNo);
        } catch (ManagerException e) {
    ...
    @Override
    public boolean rechargeSuccess(String rechargeNo, String outTradeNo, String memo, Long memberBankCardId, String bankCode, Integer payMethod)  {
        boolean result = false;
        try {
            result = rechargeLogManager.rechargeSuccess(rechargeNo, outTradeNo, memo, memberBankCardId, bankCode, payMethod);
        } catch (Exception e) {
    ...
    public boolean rechargeFailed(String rechargeNo, String outTradeNo, String memo, Long memberBankCardId, String bankCode, Integer payMethod) {
    	try {
    		return rechargeLogManager.rechargeFailed(rechargeNo, outTradeNo, memo, memberBankCardId, bankCode, payMethod);
    	} catch (Exception e) {
    ...
    private void setSecurityCard(String rechargeNo) {
        try {
            RechargeLog rechargeLogByTradeNo = rechargeLogManager.getRechargeLogByTradeNo(rechargeNo);
    ...
    public boolean withdrawProcess(String withdrawNo, String memo, Long bankCardId) {
    	boolean result = false;
    	try {           
    		result = withdrawLogManager.withdrawProcess(withdrawNo, memo, bankCardId);
    	}catch (Exception e){
    ...
    public boolean withdrawSuccess(String withdrawNo, String memo, Long bankCardId) {
    	boolean result = false;
    	try {           
    		result = withdrawLogManager.withdrawSuccess(withdrawNo, memo, bankCardId);
    	}catch (Exception e){
    		logger.error("提现异常 withdrawNo="+withdrawNo,e);
    	}
    	return  result;
    ...
    public boolean withdrawFailed(String withdrawNo, String errorMsg, Long bankCardId) {
    	try {
    		return withdrawLogManager.withdrawFailed(withdrawNo, errorMsg, bankCardId);
    	}catch (Exception e){
    		logger.error("提现异常 withdrawNo="+withdrawNo,e);
    	}
    	return  false;
	...
	public BigDecimal totalWithDraw(long memberId) {
		...
		BigDecimal result = RedisMemberClient.getWithdrawSuccessAmount(memberId);
	...
	public BigDecimal totalRecharge(long memberId) {
		BigDecimal result = RedisMemberClient.getRechargeSuccessAmount(memberId);
		return result;
	}
	...
	public List<RechargeLogDto> selectRechargeLogForPaginWeb(
		...
		try {
			List<RechargeLog> selectForPagin = rechargeLogManager.selectForPagin(map);
			List<RechargeLogDto> list = BeanCopyUtil.mapList(selectForPagin, RechargeLogDto.class);			
			return list;
		} catch (ManagerException e) {
	...
	public List<WithdrawLogDto> selectWithdrawLogForPaginWeb(Map<String, Object> map) {
		try {
			List<WithdrawLog> selectForPagin =this.withdrawLogManager.selectForPaginWeb(map);
			List<WithdrawLogDto> list = BeanCopyUtil.mapList(selectForPagin, WithdrawLogDto.class);	
			for(WithdrawLogDto dto : list){
				if (dto.getBankCardId() != null) {
					MemberBankCard memberBankCard = memberBankCardManager.selectByPrimaryKey(dto.getBankCardId());
	...
	public ResultDO<WithdrawLog> cancelToWithDrawLog(Long id, int status) {
		...
		return withdrawLogManager.cancelWithdraw(id, status);
