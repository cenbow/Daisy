...
function loadAreaByParentIdCallback(data,areaId,areaCallback){
	...
	if(data.resultList.length > 0){
		...
		if(dataList.length > 0){
			...
			$(areaUiListId).append("<li value=''>"+name+"</li>");
		}
		for(var i=0;i<dataList.length;i++){
			$(areaUiListId).append("<li value="+dataList[i].code+">"+dataList[i].name+"</li>");
