...
(function () {
    //基础方法浏览器兼容性修正
    //console
    //window.log=!$.support.opacity?function(){}:console.log.bind(console);
...
function arrowLocator(menuId) {
    if (!!menuId) {
        ...
        if (currentI.length === 1) {
            ...
            var arrow = $(menuId).find(".u-arrow-up");
            if (arrow.length > 0) {
                iWidth = arrow.width();
                left = position.left + uWidth / 2 - iWidth / 2;
                arrow.css({
...
//(function () {
////新浪维护20150702
//var sinapayBanner = $('#j-banner-sinapay20150702');
//if(sinapayBanner.length){
//    var curDate2 = new Date(environment.serverDate),
//        sinapayBannerData=sinapayBanner.data(),
//        endDate2 = new Date(sinapayBannerData.end);
//    if(curDate2<endDate2 || sinapayBannerData.handled){
//        sinapayBanner.show();
//    }else{
//    	sinapayBanner.html("");
//    }
...
//        var s2="0"+RegExp.$2,pm=RegExp.$1,a=RegExp.$3.length,b=true;
//        if(a==d+2){
//            a=s2.match(/\d/g);
