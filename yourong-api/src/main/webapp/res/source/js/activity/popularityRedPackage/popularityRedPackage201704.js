...
//     var redBagCode = getUrlParam('redBagCode');
    window.vm = new Vue({
        ...
        data: {
            ...
            phoneReg: /^13[0-9]{9}$|14[5,7]{1}[0-9]{8}$|15[0-9]{9}$|17[0,1,3,6,7,8]{1}[0-9]{8}$|18[0-9]{9}$/,
            redBagCode: getUrlParam('redBagCode'),
            redBagData: {},
            isInputPhone: false,
            showMyPackage: false,
            showBestluck: false
        ...
        created: function () {
            var self = this
            base.getAPI({
                url: self.urls.init + '?redBagCode=' + self.redBagCode,
                callback: function (data) {
                    self.redBagData = data.result
                    //console.log('init：', data)
                    var resultCodes = data.resultCodes || [];
                    // $img = $('.u-img-responsive');
                    // $href = $('.u-img-href');

                    // $('.u-img-responsive').attr("src", 'https://oss-cn-hangzhou.aliyuncs.com' + data.result.image);
                    // $('.u-img-href').attr("href", data.result.href);
        ...
        methods: {
            ...
            checkPhone: function () {
                ...
                var self = this;
                if (self.phoneReg.test(mobile)) {
                    base.getAPI({
                        url: self.urls.receiveRedbag + '?mobile=' + mobile + '&redBagCode=' + self.redBagCode,
                        callback: function (data) {
                ...
                if (hasException) {
                    ...
                    if (resultCodes && resultCodes.length > 0) {
                        if (resultCodes[0].code == '91104') {
                            ...
                            $registerBtn.find('a').text('注册使用红包').attr('href', environment.globalPath + '/mstation/register');  // 显示底部的注册按钮
                        ...
                        if (!rewardValue && hasEmpty) {
                            ...
                            if (!hasException) {
                                ...
                                if (receiveFlag !== 1) {
                                    ...
                                    $registerBtn.find('a').text('注册使用红包').attr('href', environment.globalPath + '/mstation/register'); // 显示底部的注册按钮
                                    $('#j-loading-app').removeClass('z-hidden')
                                } else {
                                    //console.log(7777)
                        ...
                        for (var i = 0; i < len; i++) {
                            ...
                            if (hasEmpty && listActivityRole == 'redPackageTop') {
                                ...
                                if (listReceiveFlag == 0) {
                                    tempStr = '<li><span class="u-list-phone">' + resultList[i].mobileStr +
                                        '</span>' + '<span class="u-list-num z-with30">' + ' <span>(新用户再得518)</span>' +
                                        resultList[i].rewardValue + '点</span><i>' + resultList[i].receiveTimeStr +
                                        '</i><div class="u-list-best">手气最佳</div></li>';
                                } else {
                                    tempStr = '<li><span class="u-list-phone">' + resultList[i].mobileStr +
                                        '</span>' + '<span class="u-list-num">' + resultList[i].rewardValue + ' 点</span><i>'
                                        + resultList[i].receiveTimeStr + '</i><div class="u-list-best">手气最佳</div></li>';
                                }
                                ...
                                // receiveFlag = 0 就显示 新用户再得30元
                                if (listReceiveFlag == 0) {
                                    listStr += '<li><span class="u-list-phone">'
                                        + resultList[i].mobileStr +
                                        '</span>' + '<span class="u-list-num z-with30 f-fs12">' + ' <span>(新用户再得518)</span>' + resultList[i].rewardValue +
                                        '点</span><i>' + resultList[i].receiveTimeStr + '</i></li>';
                                } else {
                                    listStr += '<li><span class="u-list-phone">' + resultList[i].mobileStr +
                                        '</span>' + '<span class="u-list-num">' + resultList[i].rewardValue +
                                        ' 点</span><i>' + resultList[i].receiveTimeStr + '</i></li>';
                                }
                            }
                        }
