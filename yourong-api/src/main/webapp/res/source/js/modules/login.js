 ...
 */
/*global window,$,environment,setInterval,clearInterval,formValid,define*/
...
define(['validform', 'base'], function (require, exports, module) {
    ...
    function limitCodeRequest() {
        ...
        base.xTips({
            time: 2000,
            content: CONFIG.tips.requestLimitError
        })
    }
    ...
    function timerTips(type) {
        ...
        if (type === "1") {
            // tipsPre = "短信已发送";
            tipsPre = "";
            status = "f-tips-ccc f-get-new";
        } else if (type === "2") {
            tipsPre = "已呼叫你手机";
            status = "f-tips-blue f-get-new";
        }
        ...
        $("#j-phonecode").val("").parent().removeClass("z-hide")
            ...
            '">' + tipsPre + ' <i id="j-time-count">' + CONFIG.moblieTime + '</i>s后可重获</span>');
        ...
        var timer = setInterval(function () {
            ...
            if (count !== 0) {
                ...
                clearInterval(timer);
                $(".u-tips-vercode").text("重新获取").addClass("f-tips-red j-choose-voice f-cur-point u-btn-recode").removeClass("f-tips-blue f-tips-green");
            }
        }, 1000);
    }
                            ...
                            switch (page) {
                                case "login":
                                    ...
                                    if(from) {
                                        ...
                                        loginRedirect(url,data.result);
                                    }
                                    //window.location.href = url + "/mCenter/home";
                                    break;
                                case "register":
    ...
    * @param {String} url 后端的返回页面
    * */
    function loginRedirect(path,url){
        var i1=location.href.indexOf('?from='),
            i2=location.href.indexOf('&from='),
            reUrl=path + "/mCenter/home";
        if(i1>0){
            reUrl=location.href.substr(i1+6);
        }else if(i2>0){
            reUrl=location.href.substr(i2+6);
        }
        if(reUrl.length){
...
//    }
    ...
    // 获取URL参数 cc add，春节活动登录由于后台没有做跳转，需要js这里处理
