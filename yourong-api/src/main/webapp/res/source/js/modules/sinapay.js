        ...
        xPost: function (config) {
            ...
            $.ajax({
                ...
                beforeSend: function(request) {
                    request.setRequestHeader("Accept-Version", "1.0.0");
                },
...
//验证表单填写正确统一不提示信息
    formValid.tipmsg.r = " ";
    var userNameObj=$("#j-realName"),
        identityNumberObj = $("#j-identityNumber");

    //姓名验证
    function testName() {
        var reg = /^[\u4E00-\u9FA5]{2,10}(?:·[\u4E00-\u9FA5]{2,10})*$/;
        var nameText = userNameObj.val();
        var len = nameText.length;
    ...
    // 身份证校验
    function testIdentityNumber() {
        var Wi = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2, 1]; // 加权因子;
        var ValideCode = [1, 0, 10, 9, 8, 7, 6, 5, 4, 3, 2]; // 身份证验证位值，10代表X;
        var gets = identityNumberObj.val();
        var len = gets.length;
        ...
        if (len == 0) {
            ...
            if (isValidityBrithBy15IdCard(gets)) {
                return true
            }else {
                base.xTips({
                    time: 2000,
    ...
    userNameObj.on('blur',function () {
        testName()
    });

    identityNumberObj.on('blur',function () {
        testIdentityNumber()
    })
...
//实名认证
    function showErrorMessage(data) {
        ...
        if (data.resultCodes != null && data.resultCodes != "") {
            var message = "";
            for (var x in data.resultCodes) {
                message = data.resultCodes[x].msg + message;
            }
    ...
    $("#memberIdentity_form_button").on("click", function() {
        if (testName() && testIdentityNumber()) {
            $(this).addClass("z-disabled").attr("disabled", true).val("开通中");
            var data = $("#memberIdentity_form").serializeArray();
            $.xPost({
                url: environment.globalPath + "/security/member/mstation/authIdentity",
                data: data,
                callback: function(data) {
                    ...
                    if (data.success) {
                        ...
                        window.location.href = environment.globalPath + "/mCenter/registerSuccess";
                    } else {
