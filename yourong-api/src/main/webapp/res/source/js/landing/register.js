...
define(['validform', 'base'], function (require, exports, module) {
  ...
  var base = require('base')
    ...
    $.fn.checkMobile = function (config) {
        ...
        if (val.length === 11) {
            ...
            if (isNaN(val)) {
                ...
                base.xTips({
                    time: 2000,
                    content: CONFIG.tips.cpnNotCorrect
                })
                _this.data({
                    "isCorrect": false
                });
                            ...
                            switch (code) {
                                ...
                                case 90018:
                                    ...
                                    if (!isExist) {
                                        ...
                                        base.xTips({
                                          time: 2000,
                                          content: CONFIG.tips.cpnExistedError
                                        })
                                        _this.data({
                                            "isCorrect": false
                                        });
    ...
    function timerTips(type) {
        ...
        if (type === "1") {
            tipsPre = "";
            status = "f-tips-ccc f-new-get";
        } else if (type === "2") {
            tipsPre = "已呼叫你手机";
            status = "f-tips-blue f-new-get";
        }
        ...
        $("#j-phonecode").val("").parent().removeClass("z-hide")
            .after('<span class="f-round u-tips-vercode ' + status +
        ...
        var timer = setInterval(function () {
            ...
            if (count !== 0) {
                ...
                clearInterval(timer);
                $(".u-tips-vercode").text("重新获取").addClass("f-tips-red j-choose-voice f-cur-point u-btn-recode f-new-get").removeClass("f-tips-blue f-tips-green");
            }
        }, 1000);
    }
    
    $(".m-get-gift").on('click',function () {
        var flag = false;
        flag = $('#j-cpn-pre').data('isCorrect')

        if(flag) {
            $(".m-register-box").removeClass('z-hidden')
            $(".m-cover").removeClass('z-hidden')
            $("#j-cpn").val($("#j-cpn-pre").val())
            $("#j-cpn").blur()
        }
    })

    $(".m-close-btn").on('click',function () {
        $(".m-register-box").addClass('z-hidden')
        $(".m-cover").addClass('z-hidden')
    })
    ...
    function testPnSubmitCount(obj,e) {
        var val = obj.val(),
            prevValue = obj.data("prevValue");
        if (val.indexOf(" ") !== -1) {
            val = val.replace(" ", "");
            obj.val(val);
            return false;
    ...
    function testPnLength(obj,e) {
        ...
        if (len < 11) {
            ...
            testPnSubmitCount(obj,e)
        }
    }

    function testUserPwd(obj) {
        var reg = /^(?![^a-zA-Z]+$)(?!\D+$).{6,16}$/;
        var pwdText = obj.val();
        var len = pwdText.length;

        if (len>0) {
            if (!reg.test(pwdText)) {
                base.xTips({
                    time: 2000,
                    content: "长度6-16位至少包含数字和字母"
                })
                return false
            }else {
                return true
    ...
    cpnPreObj.on("keypress", function (e) {
        var $this = $(this);
        $this.data("prevValue", $this.val());
    });
    cpnObj.on("keyup mouseup", function (e) {
        var $this = $(this);
        testPnSubmitCount($this,e)
    });
    cpnPreObj.on("keyup mouseup", function (e) {
        var $this = $(this);
        testPnSubmitCount($this,e)
    });

    //密码验证
    userPwdObj.on('blur',function (e) {
        var $this = $(this)
        testUserPwd($this)
    });
    ...
    cpnObj.on("blur", function (e) {
        var _this = $(this)
        testPnLength(_this,e)
    });
    cpnPreObj.on("blur", function (e) {
        var _this = $(this)
        testPnLength(_this,e)
    });
                ...
                $.xPost({
                    ...
                    callback: function (data) {
                        ...
                        if (status === 1) {
                            ...
                            base.xTips({
                                time: 2000,
                                content: CONFIG.tips.imgCodeError
                            })
                        }
                    }
                });
                ...
                $.xPost({
                    ...
                    callback: function (data) {
                        ...
                        if (status === 1) {
                            ...
                            base.xTips({
                                time: 2000,
                                content: CONFIG.tips.checkCodeError
                            })
                        }
                    }
                });
                        ...
                        if (data.success) {
                            ...
                            switch (page) {
                                ...
                                case "register":
                                    ...
                                    switch (pageTag) {
                                        case "landingA":
                                            window.location.href = url + "/mstation/landing/registerA-guide?register=true";
                                            break;
                                        case "inviteRegister":
                                            window.location.href = url + "/mCenter/sinapay?register=true";
                                            break;
                                        case "rewardForNew":
                                            window.location.href = url + "/landing/downloadG";
                                            break;
                                        default:
                                            window.location.href=url+'/mCenter/sinapay?register=true';
                                            break;
...
//    }
    ...
    $.fn.extend({
        ...
        fadeShow: function (config) {
            ...
            _this.on("click touchstart", "ol li:not(.z-current)", function () {
                var i = $(this).index(),
    ...
    // 查看或者隐藏密码
    $('#j-look-pwd').on('click',function () {
        var $this = $(this),
            input = $("#j-user-pwd")
        "password" ==  input.attr( "type" ) ?
            input.attr( "type", "text" ) && $this.removeClass('u-open-eyes').addClass('u-closed-eyes'):
            input.attr( "type", "password" ) && $this.removeClass('u-closed-eyes').addClass('u-open-eyes');
    })
});
