...
//			}
	...
	private  int insertMemberAndPayAccount(MemberDto record)	throws ManagerException {
		...
		if(record.getLoginSource() ==1 ){
			member.setRegisterMethod(StatusEnum.MEMBER_SOURCE_ANROID.getStatus());
		}
		if (record.getLoginSource() ==2){
			member.setRegisterMethod(StatusEnum.MEMBER_SOURCE_IOS.getStatus());
		}
		if(record.getReferCode() != null){
			Member m = memberManager.getMemberByShortUrl(record.getReferCode());
			if(m != null){
				member.setReferral(m.getId());
			}
		}else if(record.getReferralMobile() != null){
			Member m = memberManager.selectByMobile(record.getReferralMobile());
			if(m != null){
				member.setReferral(m.getId());
			}
		}
		...
		public void registerService(){
			try{
				...
				if(member != null){
					...
					memberLogin.setDevice(memberDto.getDevice());
					memberLogin.setId(member.getId());
					memberLogin.setPosition(memberDto.getPosition());
					logger.info("会员id"+member.getId()+"登陆成功");
